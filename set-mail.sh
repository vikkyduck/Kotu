...
if ! printf '%s' "$FROM" | grep -qE '^[^@[:space:]]+@[^@[:space:]]+\.[a-zA-Z]{2,}$'; then
  echo ""
  echo "❌ «$FROM» не похоже на почтовый адрес."
  if printf '%s' "$FROM" | grep -qE '^[a-z]{12,20}$'; then
    echo "   Похоже, сюда вставлен ПАРОЛЬ ПРИЛОЖЕНИЯ."
    echo "   ⚠️  Он только что отобразился на экране и попал в историю терминала —"
    echo "      отзовите его: id.yandex.ru → Безопасность → Пароли приложений."
    echo "      Пароль спрашивают на следующем шаге, там ввод не виден."
  fi
...
echo ""
echo "Шаг 2 из 2. ПАРОЛЬ ПРИЛОЖЕНИЯ для $FROM (ввод не отображается)."
printf "Пароль: "
read -rs PASS
echo
...
if printf '%s' "$PASS" | grep -q '@'; then
  echo "❌ Похоже, вы ввели адрес вместо пароля. Ничего не изменено."
  exit 1
fi
