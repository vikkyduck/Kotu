...
export interface LectureBrief {
  ...
  documentIds: number[];
  /**
   * Акценты лекции: что важнее в этот раз. Их может быть несколько — лекция
   * бывает и клинической, и исторической разом — или ни одного: тогда баланс
   * остаётся на суждение модели. Старые записи хранят здесь одиночную строку.
   */
  focus?: LectureFocus[] | LectureFocus;
  /**
   * Источники материала — два независимых выключателя. Оба сразу: выдержки
   * библиотеки И веб-исследование складываются в общий материал. Ни одного:
   * лекция пишется по знаниям модели с честными пометками. Поле mode осталось
   * от старой записи «или/или» и читается только для совместимости.
   */
  useLibrary?: boolean;
  useResearch?: boolean;
