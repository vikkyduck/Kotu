...
- `GET /api/healthz` → `{"status":"ok","archive":"ok"}`. `archive:"off"` — архив не включился: сервер работает, но удаления отклоняет (503), а `deploy.sh` такую выкатку бракует
- Бэкап: `/opt/backups/data/archive` — зеркало архива без `--delete`. Снимки `/opt/backups/predeploy` больше не ротируются
- `drizzle push` схему `archive` не видит и не трогает (управляет только `public`). Если push когда-нибудь пересоздаст таблицу, триггер вернётся на следующем старте сервера
