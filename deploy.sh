...
# следующей ночью ушёл бы из зеркала без следа. Дампы базы ротируются через
# 14 дней, а строки, испорченные мимо триггеров (drizzle push, ручной SQL), в
# archive.rows не попадут. Поэтому перед каждой выкаткой — отдельный снимок,
