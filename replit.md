...
- Transcription pipeline (`artifacts/api-server/src/lib/transcription.ts`): audio → ffmpeg normalizes to mono 16 kHz mp3 and splits into 10-min chunks (`transcribeRecording`) → each chunk transcribed via `gpt-4o-transcribe` (filename passed for format detection, bounded concurrency) → per-chunk structuring pass (json_object) that splits into segments, optionally redacts patient names («скрыто») and labels speakers ("Вы"/"Собеседник") → segments stitched in order.
...
- Transcription models cap each request at 1500 s (25 min) and 25 MB — the duration cap is the real bottleneck, so long recordings are split server-side with ffmpeg (available in the env). Upload uses multer disk storage with a 300 MB cap; oversized files get a friendly Russian error on `LIMIT_FILE_SIZE` (413). Audio is split in a temp dir and deleted after transcription.
