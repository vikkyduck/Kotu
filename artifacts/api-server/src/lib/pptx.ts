 ...
 * в двух гарнитурах. Заголовки — Manrope с bold. Надзаголовки и музейная
 * подпись — Medium, как в PDF; нет такого начертания на машине — выйдет
 * обычный Manrope, как было.
 */
const FONT = "Manrope";
...
/** Палитра пакета может прийти неполной — тогда брендбук. */
function makeStyle(pack: StylePack | null): Style {
  return { color: (name) => hex(slideColor(pack?.palette, name)) };
}

/** Арабский folio внизу листа (брендбук: римские — разделам, арабские — folio). */
function addFolio(out: PptxGenJS.Slide, idx: number): void {
...
function addDivider(out: PptxGenJS.Slide, c: SlideContent, img: string | null, st: Style): void {
  ...
  runs.push({
    text: c.title ?? "",
    options: { fontFace: FONT, bold: true, fontSize: T.dividerTitle, color: TEXT, lineSpacingMultiple: L.title },
  });
...
function addQuote(out: PptxGenJS.Slide, c: SlideContent, img: string | null, st: Style, side: "left" | "right"): void {
  ...
  if (c.attribution) {
    runs.push({
      text: c.attribution,
      options: { fontFace: FONT, fontSize: T.attribution, color: TEXT },
    });
  }
...
function addComparison(out: PptxGenJS.Slide, c: SlideContent, img: string | null, st: Style): void {
  ...
  cards.slice(0, MAX_CARDS).forEach((card, i) => {
    const runs: PptxGenJS.TextProps[] = [
      {
        text: card.title,
        options: { fontFace: FONT, bold: true, fontSize: T.cardTitle, color: TEXT, breakLine: true, paraSpaceAfter: 8 },
      },
      {
        text: card.body,
        options: { fontFace: FONT, fontSize: T.cardBody, color: TEXT, lineSpacingMultiple: L.body },
      },
    ];
    out.addText(runs, {
...
function addFinal(out: PptxGenJS.Slide, c: SlideContent, st: Style, idx: number): void {
  ...
  const runs: PptxGenJS.TextProps[] = [
    {
      ...
      options: { fontFace: FONT, bold: true, fontSize: T.finalTitle, color: TEXT, lineSpacingMultiple: L.final },
    },
  ];
  if (c.subtitle && c.title) {
    ...
    runs.push({
      text: c.subtitle,
      options: { fontFace: FONT, fontSize: T.finalSubtitle, color: TEXT },
    });
  }
...
function addDiagram(out: PptxGenJS.Slide, c: SlideContent, spec: DiagramSpec, st: Style): void {
  ...
  if (spec.kind === "flow") {
    ...
    spec.items.forEach((it, i) => {
      ...
      const runs: PptxGenJS.TextProps[] = [
        { text: it.label, options: { fontFace: FONT, bold: true, fontSize: T.nodeLabel, color: TEXT } },
      ];
      if (it.sub) {
        runs[0]!.options!.breakLine = true;
        ...
        runs.push({
          text: it.sub,
          options: { fontFace: FONT, fontSize: T.nodeSub, color: TEXT, lineSpacingMultiple: L.nodeSub },
        });
      }
  ...
  items.forEach((it, i) => {
    ...
    const runs: PptxGenJS.TextProps[] = [
      {
        text: it.label,
        options: { fontFace: FONT, bold: true, fontSize: T.nodeLabel, color: TEXT, breakLine: true, paraSpaceAfter: 8 },
      },
    ];
    if (it.sub) {
      runs.push({
