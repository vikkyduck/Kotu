...
/**
 * Метки скрытых имён ([[PER1]], [[LOC2]]) из копий расшифровок — для модели,
 * а человеку в выдаче поиска и цитатах источников они ничего не говорят.
 * Показываем так же, как на экране записи. Сам текст копии не трогаем:
 * номера меток помогают модели не путать разных людей.
 */
export function hideLabels(text: string): string {
