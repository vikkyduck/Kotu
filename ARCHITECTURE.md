...
| Какие модели | Распознавание и поиск имён — **только локальные** (Whisper, NER); оформление — зарубежная модель по тексту **без имён и мест** | Можно зарубежные через транзит |
| Покидает РФ | Аудио и настоящие имена — **никогда**; текст сеанса с метками вместо имён и мест — да, для оформления | Да, осознанно |
...
- маскировка **всегда**, независимо от галочки «скрывать имена» (галочка влияет только на показ);
- если локальный NER недоступен, текст **не отправляется** — задача ждёт повтора (fail-closed),
  никакой «эвристики на крайний случай»;
- аудио и таблица соответствий меток и имён не покидают сервер в РФ.

Если ограничения Timeweb станут мешать, переезд платформы на зарубежный сервер владелица
допускает как запасной вариант — это отдельное решение, сейчас не принято.
...
(2026-09-23) она безусловная и fail-closed — см. решение выше.
...
- Бэкапы: ночной `ops/kotu-backup.sh` (дамп базы на 14 дней + зеркало файлов) и снимок перед
  каждым деплоем (`deploy.sh`: дамп `kotu` + жёсткие ссылки на library/uploads/decks). Зеркало
  повторяет удаления, поэтому защиту от собственной ошибки даёт именно снимок перед деплоем.
