...
Исторический документ, 2026-07-22: как устроено сейчас — `ГДЕ-ЧТО.md` и код.

---

## 0. Что уже есть (инвентаризация на 2026-07-22)

Исторический раздел, 2026-07-22.

**Работает в проде** (тогда — `http://5.129.198.180:8091`; с 2026-07-30 — https://psy3107.ru, VDS Timeweb, Москва):
...
|---|---|---|---|---|---|
| С1 | Кот | Загружает запись сеанса/лекции | Получить текст | Расшифровка (имена скрываются по галочке) | ✅ готово |
...
```mermaid
graph LR
  A[Кот в браузере] --> B[nginx psy3107.ru :443]
  B --> C[API-сервер :5010]
  C --> D[(Postgres РФ)]
...
users
  ...
  password_hash text                   обяз.                      — scrypt, "salt:hash" в hex
...
REST, ответы — JSON, ошибка — `{message}` по-русски. Генератор контракта (`lib/api-spec`, Orval,
`lib/api-zod`) убран: его использовал один раздел из восьми. Фронт ходит обычным `fetch` через
`artifacts/kot/src/lib/http.ts`. Все ручки, кроме `/auth/*` и `/healthz`, требуют сессионную cookie.

Сами ручки — в `artifacts/api-server/src/routes/*`: таблица, что стояла здесь, отстала от кода.
...
### ✅ Дефект 2 (закрыт): у платформы не было авторизации

Исторический раздел. Вход сделан (`routes/auth.ts`, `lib/auth.ts`): почта и пароль (scrypt),
сессия в httpOnly-cookie, ограничение попыток, HTTPS на https://psy3107.ru; старый адрес :8091
только редиректит.
...
**Как чинить:** вход по email+паролю, сессия в httpOnly-cookie, ограничение попыток
...
Где что лежит в коде — `ГДЕ-ЧТО.md`, раздел «Код».

Правила размещения (чтобы не расползлось):
- **UI** — только в `artifacts/kot`, никаких обращений к моделям с фронта;
- **маршрут** принимает, проверяет и ставит задачу; долгая работа — в обработчиках очереди
  (`artifacts/api-server/src/lib/handlers`);
- **работа с БД** — через схему `lib/db`;
- **обращения к моделям** — только с сервера, по правилу двух зон (§10);
- **типы** — из схемы БД, руками не дублируем (фронт берёт их через `import type`).
