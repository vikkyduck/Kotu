...
| **Генерация иллюстраций** | **Gemini** (`gemini-3-pro-image`) | Проверено 2026-07-30 живой генерацией: стиль гравюр воспроизводит, 35 с на картинку. Запасная `gemini-2.5-flash-image` — 15 с, рисунок площе |
...
### Стилевой пакет «Академический сюрреализм» (задан автором)

Эстетика времён зарождения психоанализа — кабинет Фрейда, старые медицинские атласы — смешанная
с визуальным языком сновидений.

**Мастер-промпт** (добавляется к сюжету каждой картинки):

```
in the style of vintage academic etching and surrealist illustration on aged textured paper.
Muted color palette of sepia, deep indigo, burnt umber, and charcoal gray. The aesthetic is
intellectual, dreamlike, sophisticated, and symbolic, reminiscent of 19th-century medical
engravings mixed with the works of Max Ernst and Odilon Redon. Fine lines, cross-hatching
textures, watercolor washes. A feeling of psychological depth and historical archive.
No cartoon elements, no bright colors, no photorealism.
```

**Запрет** (`negative`): `cartoon, anime, 3d render, cheerful, bright colors, saturated, childlike,
simplistic, corporate Memphis art, low resolution, blurry, text, watermark`

К запрету автора добавлено от нас: **`frame, border`** — проверка 2026-07-30 показала, что модель
самовольно дорисовывает рамку, а на слайде она съедает поле и мешает вёрстке.

**Референсы художников** — если стиль «уплывает», в промпт добавляются имена: Max Ernst
(коллажность), Odilon Redon (сновидческая дымка), Albrecht Dürer (качество гравюры),
Giovanni Battista Piranesi (сложные пространства).

**Палитра:** сепия, охра, глубокий индиго, жжёная умбра, угольно-серый. Яркое — только акцентом
для аффекта или либидо: приглушённый карминный, тусклое золото.
...
Отсюда два правила: выбор модели живёт в конфигурации (`MODEL_*`), а у картинок есть **запасная**
модель (`MODEL_IMAGE_FALLBACK`) — если основная молчит, конвейер не встаёт, а дорисовывает
запасной. Разница в качестве меньше, чем разница между «есть картинка» и «нет картинки».
...
decks
  ...
  status             drafting | storyboard_ready | rendering | ready | error
  status_message, error, created_at

deck_slides
  id, deck_id, ord
  layout             title | section | bullets | two-cards | quote | diagram | image-full | closing
  content   jsonb    заголовок, надзаголовок, тезисы, карточки
  notes              заметки докладчику
  image_brief        заказ художнику (NULL — картинка не нужна)
  diagram_spec jsonb описание схемы (NULL — схемы нет)
  image_id           готовая иллюстрация

images
  id, deck_id, slide_id
  prompt             итоговый промпт со стилевым префиксом
  path               файл на диске
  status             queued | drawing | ready | rejected | error
  attempt            номер попытки (потолок — 2)
  verdict            что сказала приёмка

style_packs
  id, name
  prompt_prefix      жёсткая часть промпта: гравюра, сепия, штриховка
  negative           чего не должно быть: текст, рамки, подписи
  palette
```
...
Ограниченный набор макетов (по референсу «Воркшоп_Enterprise»: надзаголовок, крупный заголовок,
карточки, сноска с источником, заметки докладчику) — залог того, что серия выглядит цельно.
